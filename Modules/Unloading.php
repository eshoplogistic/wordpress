...
class Unloading implements ModuleInterface {
	...
	public function esl_form_in_admin_bar() {
		...
		if( ($pageType == 'shop_order' && $pagenow == 'post.php') || ($pageType == 'shop_order' && $pagenow == 'admin.php') ) {
			...
			$fieldDelivery = $methodDelivery->exportFields(mb_strtolower($typeMethod['name']), $this->shippingMethods);
	...
	public function params_delivery_init($data){
		...
		if(!$result->hasErrors()){
			...
			if($data['delivery_id'] == 'sdek' && isset($shippingMethods['answer']['order']['id'])){
				...
				$eshopLogisticApi = new EshopLogisticApi(new WpHttpClient());
				$resultGet = $eshopLogisticApi->apiExportCreateSdek($data);
		...
		if(isset($data['products'])){
			foreach ($data['products'] as $item){
				...
				$defaultFields['places'][] = array(
					...
					'count' => $item['quantity'],
					'price' => $item['total'],
					'weight' => $shippingHelper->weightOption($item['weight']), //Вес, в кг.
					'dimensions' => $shippingHelper->dimensionsOption($item['width']).'*'.$shippingHelper->dimensionsOption($item['length']).'*'.$shippingHelper->dimensionsOption($item['height']), //Габариты. Формат: строка вида «Д*Ш*В», в сантиметрах. Например: 15*25*10
