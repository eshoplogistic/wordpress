...
class ExportFileds {
	...
	public function renderTabFields( $carrierSlug ) {
		...
		if ( $carrierSlug !== 'halva' ) {
			...
			$html .= '<h4>' . esc_html__( 'Дополнительные настройки ТК.', 'eshoplogistic' ) . '</h4>';
			$html .= '<div class="form-group row align-items-center mb-3">
				<label class="col-sm-5 col-form-label">' . esc_html__( 'Способ доставки до терминала ТК по умолчанию', 'eshoplogistic' ) . '</label>
				<div class="col-sm-5">
					<select class="form-control" form="eslExportForm" name="' . esc_attr( $pickUpKey ) . '">
						<option value="0" ' . selected( $pickUp, '0', false ) . '>' . esc_html__( 'Сами привезём на терминал транспортной компании', 'eshoplogistic' ) . '</option>
						<option value="1" ' . selected( $pickUp, '1', false ) . '>' . esc_html__( 'Груз заберёт транспортная компания', 'eshoplogistic' ) . '</option>
					</select>
				</div>
			</div>';
		}
		...
		if ( $html === '' ) {
			$html = '<p>' . esc_html__( 'Дополнительных настроек для этой службы нет.', 'eshoplogistic' ) . '</p>';
		}
